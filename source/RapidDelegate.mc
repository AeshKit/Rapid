...
class RapidDelegate extends Ui.BehaviorDelegate {
    ...
    var timeThen as Number  = 0;
    var sideTimeThenWhite;
    var sideTimeThenBlack;
    var totalTimeTemp as Number = 0;
    ...
    function drawLegacyMenu() {
        ...
        menu.addItem(Disk.getValue("backlight") ?
    ...
    function onTap(clickEvent) {
        if((Globals.changingStartingTimeWithButton || Globals.changingSideWithButton != 'n') && Ui.BehaviorDelegate has :onDrag) {
        ...
        if(Globals.hasGameBeenStopped && clickEvent.getCoordinates()[0] > screenWidthOver155) {
        ...
        if(!Globals.gameStarted) {
            switch(dragEvent.getType()) {
                case 0: // New tap
                    ...
                    // The if/else block below is for parity with dragging while changing the time with a button
                    // Otherwise, the time will be reset when starting a drag back to its original value
                    ...
                    switch(Globals.axis) {
                        case "N":
                            ...
                            if(absX > swipeThreshold || absY > swipeThreshold) {
                                ...
                                Ui.requestUpdate(); // Allows rapidView to start the timer that updates the screen automatically
                            }
                            break;
                        case "X":
                            if(!Globals.hasGameBeenStopped || Globals.isWhitesTurn) {
                                Globals.incrementHoldingVar = Globals.whiteIncrement - deltaX / (screenHeightOver10);
                            } else {
                                Globals.incrementHoldingVar = Globals.blackIncrement - deltaX / (screenHeightOver10);
                            }
                        ...
                        case "Y":
                            if(!Globals.hasGameBeenStopped) {
                                Globals.startingTimeHoldingVar = (Globals.startingTimeInSeconds * 1000) + (deltaY / (screenHeightOver10)) * 15000;
                                ...
                                base = Globals.isWhitesTurn ? Globals.whiteTimeLeft : Globals.blackTimeLeft;
                                Globals.changingTimeHoldingVar = base + (deltaY / (screenHeightOver60)) * 5000;
    ...
    function handleHorizontalDragStop() as Void {
        Globals.interestingCountingVariable = 0;
        if(!Globals.hasGameBeenStopped) {
            Globals.whiteIncrement = Globals.incrementHoldingVar;
            Globals.blackIncrement = Globals.incrementHoldingVar;
    ...
    function handleVerticalDragStop() {
        Globals.interestingCountingVariable = 0;
        if(!Globals.hasGameBeenStopped) {
            ...
            Globals.whiteTimeLeft =  Globals.changingTimeHoldingVar;
            Globals.whiteTimeLeft += Globals.hasGameBeenStopped ? 0 : Globals.whiteIncrement * 1000;
        } else {
            Globals.blackTimeLeft =  Globals.changingTimeHoldingVar;
            Globals.blackTimeLeft += Globals.hasGameBeenStopped ? 0 : Globals.blackIncrement * 1000;
        }
    }
    ...
    function onKey(keyEvent) {
        switch(keyEvent.getKey()) {
            ...
            case Ui.KEY_ENTER:
            case Ui.KEY_START:
                // TODO: Check if the 'Too Many Timers Error' happens on real hardware.
                if(Globals.gameStarted) { changeSidesAndAddIncrement(); }
                    ...
                    handleHorizontalDragStop();
                    Globals.changingIncrementWithButton = false;
                } else if(Globals.isDragging) { break; }
                else { startTimer(); }
                break;
            case Ui.KEY_DOWN:
                if(Globals.gameStarted) {
                ...
                if(Globals.changingIncrementWithButton) {
                    ...
                    } else if(Globals.incrementHoldingVar > 30) {
    ...
    function startTimer() {
        ...
        timer.start(method(Globals.isWhitesTurn ? :whiteDecrement : :blackDecrement), 50, true); // wow that looks confusing
    ...
    function stopTimer() {
        var timeNow   = System.getTimer() & Constants.UINT32_MAX;
        var timeDelta = (timeNow - timeThen).abs();
        timeThen = timeNow;
    ...
    function timeOutCall() {
        if(Disk.getValue("scalingTimeOut")) {
            Ui.switchToView(new $.TimeOutView(), new $.TimeOutDelegate(Dictionary), Ui.SLIDE_IMMEDIATE);
        } else {
